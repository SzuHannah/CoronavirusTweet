...
lmlib<-lm(ylib~as.matrix(libfaZ))%>%summary
lmlib
...
result<-capture.output(glmls[["liberals"]])
glmoutput<-lapply(glmls, function(x){
  capture.output(x)
})
#glm, family = "gaussian" is the same as linear model
lmls<-lapply(seq_along(covidflock_conf),function(x){
  y<-covidflock_conf[[x]]$log_conf
  z<-falsflock[[x]]$Z
  lm(y~as.matrix(z))%>%summary
})
names(lmls)<-c("liberals","conservatives","media","issue-centric","pop culture","academia")
```
...
covidus_casedeathsel
#for reporting data source
joinedcovid<-covidusspreadsel%>%left_join(covidus_casedeathsel)
joinedcovid
